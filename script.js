...
// Función para manejar el header que cambia con scroll
window.addEventListener('scroll', function() {
  const header = document.getElementById('main-header');
  
  // Aplicar clase 'compact' cuando el scroll pasa de 50px
...
function calcularTotales() {
  ...
  const totalHoras = modulos.reduce((total, modulo) => total + modulo.horas, 0);
  
  // Calcular costo total (horas * costo por hora)
  const costoPorHora = 20;
  const costoTotal = totalHoras * costoPorHora;
